...
public class Tablero {
    ...
    public void ponerFicha(int x, int y, int color) {
        tablero[y][x] = color;
        System.out.println("Poner ficha en: " + x + "," + y + " Color:"+ color);
    ...
    public int obtenerCasilla(int fila, int columna) {
        System.out.println("Casila obtenida: " + fila + "," + columna);
    ...
    private void rellenarColor(int f, int c, int dy, int dx, int color) {
        int fila = f + dy;
        int columna = c + dx;
        System.out.println("Rellenando con nuevo color");
    ...
    public boolean movimientoCorrecto(Movimiento m, int color) {
        ...
        if (tablero[m.fila][m.columna] != 0) {
            System.out.println("Movimiento incorecto");
            correcto = false;
        } else {
    ...
    public int ganador() {
        ...
        if (numFichas[1] > numFichas[2]) {
            return 1;
        } else if (numFichas[2] > numFichas[1]) {
            return 2;
        } else {
            return 0;
        }
